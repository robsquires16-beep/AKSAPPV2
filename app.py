...
def fallback_copy(service_area, business_name, phone, tone, post_style, seo_city, booking_link, offer_text):
    ...
    return {
        ...
        "cta": cta,
        "share_text": f"{post}\n\n{cta}",
        "note": "Generated in fallback mode because no OpenAI API connection was available.",
    }
...
"""
    ...
    try:
        ...
        data = fallback_copy(service_area, business_name, phone, tone, post_style, seo_city, booking_link, offer_text)
        data["note"] = "The model response could not be parsed cleanly, so a safe fallback version was used."
        return data
